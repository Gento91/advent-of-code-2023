function isPossible(game){
    ...
    for (let i = 0; i < sets.length; i++){
        let nbRed = 0, nbBlue = 0, nbGreen = 0
        let colors = sets[i].split(", ")
        
        colors.forEach(color => {
            let [nbr, clr] = color.split(' ')
            if (clr.includes('red')) {
                nbRed += parseInt(nbr)
            } else if (clr.includes('blue')) {
                nbBlue += parseInt(nbr)
            } else if (clr.includes('green')) {
                nbGreen += parseInt(nbr)
            }
        })
        if (nbRed > 12 || nbBlue > 14 || nbGreen > 13){
            return false
        }
    }
    return true
